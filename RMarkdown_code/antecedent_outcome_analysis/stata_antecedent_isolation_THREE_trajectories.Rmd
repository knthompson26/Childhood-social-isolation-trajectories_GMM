...
# Reformat data frames
 ...
 - Have not included multiple testing correction here
  ...
  mutate(
    block = 
      case_when(row_number() < 7 ~ "Social block",
                ...
                row_number() < 22 ~ "Parent block",
                row_number() < 25 ~ "Neuro block",
                row_number() < 29 ~ "Emo/behave block",
                row_number() < 35 ~ "Social block",
                row_number() < 42 ~ "Home block",
                row_number() < 50 ~ "Parent block",
                row_number() < 53 ~ "Neuro block",
                row_number() > 52 ~ "Emo/behave block")) %>%
...
univariate.reg.table <- kable(dat.stata.individual, digits = 2)
...
dat.stata.individual.decreasing <- dat.stata.individual %>%
...
dat.stata.individual.increasing <- dat.stata.individual %>%
...
dat.stata.all.siguni <- dat.stata.all.siguni.raw[c(112:215,221:324),] %>%
  ...
  mutate(
    block = case_when(
              row_number() < 17  ~ "Social block",
              row_number() < 45 ~ "Home block",
              row_number() < 77 ~ "Parent block",
              row_number() < 89 ~ "Neuro block",
              row_number() < 105 ~ "Emo/behave block",
              row_number() < 121 ~ "Social block",
              row_number() < 149 ~ "Home block",
              row_number() < 181 ~ "Parent block",
              row_number() < 193 ~ "Neuro block",
              row_number() > 192 ~ "Emo/behave block")) %>%
...
```{r stata all table}
multivariate.siguni.reg.table <- kable(dat.stata.all.siguni)
...
# social
individual.and.multi.sig.social <- filter(individual.and.multi.sig, block == "Social block") %>% 
  arrange(desc(RRR)) 

individual.and.multi.sig.social$Variable <- factor(individual.and.multi.sig.social$Variable, levels = paste(unique(paste(individual.and.multi.sig.social$Variable)), sep = ","))

#home
individual.and.multi.sig.home <- filter(individual.and.multi.sig, block == "Home block") %>% 
  arrange(desc(RRR)) 

individual.and.multi.sig.home$Variable <- factor(individual.and.multi.sig.home$Variable,levels = paste(unique(paste(individual.and.multi.sig.home$Variable)),sep = ","))

#parent
individual.and.multi.sig.parent <- filter(individual.and.multi.sig, block == "Parent block") %>% 
  arrange(desc(RRR))

individual.and.multi.sig.parent$Variable <- factor(individual.and.multi.sig.parent$Variable,levels = paste(unique(paste(individual.and.multi.sig.parent$Variable)),sep = ","))

#neuro
individual.and.multi.sig.neuro <- filter(individual.and.multi.sig, block == "Neuro block") %>% 
  arrange(desc(RRR))

individual.and.multi.sig.neuro$Variable <- factor(individual.and.multi.sig.neuro$Variable,levels = paste(unique(paste(individual.and.multi.sig.neuro$Variable)),sep = ","))

#emo/behave
individual.and.multi.sig.emo <- filter(individual.and.multi.sig, block == "Emo/behave block") %>% 
  arrange(desc(RRR))

individual.and.multi.sig.emo$Variable <- factor(individual.and.multi.sig.emo$Variable,levels = paste(unique(paste(individual.and.multi.sig.emo$Variable)),sep = ","))
...
# Plot for univariate regressions - with significant multivariate associations indicated
