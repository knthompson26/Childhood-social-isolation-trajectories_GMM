...
{ cat(i)
  ...
  if (i < 11) {
   ...
   logistic.output.outcome.full <- logistic.output.outcome.full %>%
      ...
      mutate(Variable = outcome.dependent.logistic.name.list[[i]])
                #  %>% mutate(p.value.adjusted = p.value)                                  # no correction needed here as only one variable
  }
   ...
   logistic.output.outcome.full <- logistic.output.outcome.full %>% 
    ...
    Significance = 
      if_else(
        p.value < 0.05, # WITHOUT FDR CORRECTION
        1,
        0
      ) %>%
    recode_factor(
...
kable(data.outcome.logistic, digits = c(2,2,2,2,2,3,2,2))
```

```{r bind and order rows stata}
# mental health
data.outcome.logistic.mental <- filter(data.outcome.logistic, block == "Mental health block") %>% 
  arrange(desc(OR)) 

data.outcome.logistic.mental$`Dependent variable` <- factor(data.outcome.logistic.mental$`Dependent variable`, levels = paste(unique(paste(data.outcome.logistic.mental$`Dependent variable`)), sep = ","))

# employment prospects
data.outcome.logistic.employ <- filter(data.outcome.logistic, block == "Employment block") %>% 
  arrange(desc(OR))

data.outcome.logistic.employ$`Dependent variable` <- factor(data.outcome.logistic.employ$`Dependent variable`,levels = paste(unique(paste(data.outcome.logistic.employ$`Dependent variable`)),sep = ","))
...
{ cat(i)
  ...
  linear.output.outcome.full <- broom::tidy(coeftest(linear.reg.outcome,
    ...
    dplyr::rename(robust.std.error = std.error) %>%                      # rename the standard error variable to show that it's robust
    mutate(Class = c("Increasing", "Decreasing"))                        # Create new formatted class variable
...
```{r table for combines linear dataframes}
kable(data.outcome.linear, digits = c(2,2,2,2,2,3,2,2))
```
...
kable(data.outcome.linear.zscore)
```

### Getting multiple testing correction (BH FDR) p values

```{r multiple testing correction for linear models}
data.outcome.linear.zscore.decreasing <- data.outcome.linear.zscore %>%
...
data.outcome.linear.zscore.increasing <- data.outcome.linear.zscore %>%
  filter(Class == "Increasing") %>%
  mutate(
    p.value.adjusted = 
           p.adjust(p.value, 
                    method = "BH",
...
kable(data.outcome.linear.zscore.fdr, digits = 2)
```

```{r bind and order rows stata}
# mental health
data.outcome.linear.mental <- filter(data.outcome.linear.zscore, block == "Mental health block") %>% 
  arrange(desc(estimate)) 

data.outcome.linear.mental$`Dependent variable` <- factor(data.outcome.linear.mental$`Dependent variable`, levels = paste(unique(paste(data.outcome.linear.mental$`Dependent variable`)), sep = ","))

# physical health
data.outcome.linear.physical <- filter(data.outcome.linear.zscore, block == "Physical health block") %>% 
  arrange(desc(estimate)) 

data.outcome.linear.physical$`Dependent variable` <- factor(data.outcome.linear.physical$`Dependent variable`,levels = paste(unique(paste(data.outcome.linear.physical$`Dependent variable`)),sep = ","))

# coping and functioning
data.outcome.linear.coping <- filter(data.outcome.linear.zscore, block == "Coping and functioning block") %>% 
  arrange(desc(estimate))

data.outcome.linear.coping$`Dependent variable` <- factor(data.outcome.linear.coping$`Dependent variable`,levels = paste(unique(paste(data.outcome.linear.coping$`Dependent variable`)),sep = ","))

# employment prospects
data.outcome.linear.employ <- filter(data.outcome.linear.zscore, block == "Employment block") %>% 
  arrange(desc(estimate))

data.outcome.linear.employ$`Dependent variable` <- factor(data.outcome.linear.employ$`Dependent variable`,levels = paste(unique(paste(data.outcome.linear.employ$`Dependent variable`)),sep = ","))
...
## Panel plot for linear and logistic regression 
- linear using z scores 

```{r panel plots for factor and symptoms}
library(ggpubr)
outcome.linear.separate.plot.zscore <- outcome.linear.separate.plot.zscore + 
...
outcome.logistic.separate.plot <- outcome.logistic.separate.plot + 
...
# combined.linear.logistic <- ggarrange(outcome.linear.separate.plot.zscore, 
#                                       outcome.logistic.separate.plot + 
#                                         font("x.text", 
#                                              size = 10),
#                                       ncol = 2, 
...
combined.linear.logistic <- outcome.linear.separate.plot.zscore + 
  outcome.logistic.separate.plot + 
  plot_layout(ncol = 1, 
              guides = 'collect') + 
...
#png
ggsave(
  "combined.outcome_plot.png",
  plot = combined.linear.logistic,
  device = "png",
  path = paste0(graph_save_data_path,"outcomes/final"),
  width = 9,
  height = 13
...
```{r save logistic spearate plot ant control}
if(posterior.cut == FALSE){
# save
ggsave(
...
```{r save linear individual regressions z score ant control}
if(posterior.cut == FALSE){
# save
ggsave(
...
```{r combine linear pairwise dataframes}
...
```{r table combined linear pairwise dataframes}
kable(data.outcome.linear.zscore.pairwise)
```
...
```{r save linear regressions z score pairwise}
if(posterior.cut == FALSE){
# save
ggsave(
...
```{r table combine logistic dataframes pairwise control antecedent}
kable(data.outcome.logistic.pairwise.ant_control)
```
...
```{r logistic pairwise ant control}
if(posterior.cut == FALSE){
# save
ggsave(
...
```{r table combined linear dataframes pairwise control antecedent}
kable(data.outcome.linear.zscore.pairwise.ant_control)
```
...
```{r save linear z score pairwise ant control}
if(posterior.cut == FALSE){
# save
ggsave(
