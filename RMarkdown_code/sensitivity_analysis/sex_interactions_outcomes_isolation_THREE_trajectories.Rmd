...
ANALYSIS PLAN - SAME AS THE OUTCOME SCRIPT
...
Significant interaction between sex and class membership for "Cannabis dependence symptoms", "BMI", "Log CRP", "PSQI score". 
...
```{r service use for boys}
logistic.formula.outcome.service.boys <- as.formula(service_use_18 ~ class_reordered + SES_ordered)

logistic.reg.outcome.service.boys <- glm(logistic.formula.outcome.service.boys, data = dat.male, family = "binomial")
  
# robust standard error estimation - account for non-independence of twin sample
logistic.output.outcome.full.service.boys <- summ(logistic.reg.outcome.service.boys,
                                       scale = FALSE,          # standardized regression coefficients
                                       confint = TRUE,        # report confidence intervals over standard errors
                                       ci.width = 0.95,       # desired confidence interval
                                       robust = "HC0",        # heteroskedasticity-robust standard errors instead of conventional SEs
...
logistic.output.outcome.full.service.boys <- as.data.frame(logistic.output.outcome.full.service.boys$coeftable)[-c(1,4,5),] %>% # delete SES coefficients
  mutate(Class = c("Increasing", "Decreasing")) %>% # Create new formatted class variable
  mutate(block = "Mental health block") %>%
  mutate(Variable = "Service use: Yes") %>%
#  mutate(p.value.adjusted = p.adjust(p, method = "fdr", n = 2)) %>%
  mutate(Significance = if_else(
    p < 0.05,
    1,
...
# table
kable(logistic.output.outcome.full.service.boys)
```

```{r service use for girls}
logistic.formula.outcome.service.girls <- as.formula(service_use_18 ~ class_reordered + SES_ordered)

logistic.reg.outcome.service.girls <- glm(logistic.formula.outcome.service.girls, data = dat.female, family = "binomial")
  
# robust standard error estimation - account for non-independence of twin sample
logistic.output.outcome.full.service.girls <- summ(logistic.reg.outcome.service.girls,
                                       scale = FALSE,          # standardized regression coefficients
                                       confint = TRUE,        # report confidence intervals over standard errors
                                       ci.width = 0.95,       # desired confidence interval
                                       robust = "HC0",        # heteroskedasticity-robust standard errors instead of conventional SEs
...
logistic.output.outcome.full.service.girls <- as.data.frame(logistic.output.outcome.full.service.girls$coeftable)[-c(1,4,5),] %>% # delete SES coefficients
  mutate(Class = c("Increasing", "Decreasing")) %>% # Create new formatted class variable
  mutate(block = "Mental health block") %>%
  mutate(Variable = "Service use: Yes") %>%
#  mutate(p.value.adjusted = p.adjust(p, method = "fdr", n = 2)) %>%
...
# table
kable(logistic.output.outcome.full.service.girls)
```
...
```{r CRP model for boys}
linear.formula.outcome.crp.boys <- as.formula(CRP_log_18 ~ class_reordered)

linear.reg.outcome.crp.boys <- lm(linear.formula.outcome.crp.boys, data = dat.male)

# robust standard error estimation - account for non-independence of twin sample
linear.output.outcome.full.crp.boys <- broom::tidy(coeftest(linear.reg.outcome.crp.boys,      # reports coefficients for new covariance matrix
                                                                    vcov = vcovHC(linear.reg.outcome.crp.boys,             # estimates a robust covariance matrix
                                                                    type = "HC0",                            # gives White's estimator 
                                                                    cluster = "familyid")),                  # clusters by familyid
                                               conf.int = TRUE, 
                                               conf.level = 0.95)[-c(1),] %>%
...
# table
kable(linear.output.outcome.full.crp.boys)
```

```{r CRP model for girls}
linear.formula.outcome.crp.girls <- as.formula(CRP_log_18 ~ class_reordered)

linear.reg.outcome.crp.girls <- lm(linear.formula.outcome.crp.girls, data = dat.female)

# robust standard error estimation - account for non-independence of twin sample
linear.output.outcome.full.crp.girls <- broom::tidy(coeftest(linear.reg.outcome.crp.girls,      # reports coefficients for new covariance matrix
                                                                    vcov = vcovHC(linear.reg.outcome.crp.girls,             # estimates a robust covariance matrix
                                                                    type = "HC0",                            # gives White's estimator 
                                                                    cluster = "familyid")),                  # clusters by familyid
                                               conf.int = TRUE, 
                                               conf.level = 0.95)[-c(1),] %>%
...
# table
kable(linear.output.outcome.full.crp.girls)
```

### PSQI score 

```{r PSQI model for boys}
linear.formula.outcome.PSQI.boys <- as.formula(PSQI_global_score_18 ~ class_reordered)

linear.reg.outcome.PSQI.boys <- lm(linear.formula.outcome.PSQI.boys, data = dat.male)

# robust standard error estimation - account for non-independence of twin sample
linear.output.outcome.full.PSQI.boys <- broom::tidy(coeftest(linear.reg.outcome.PSQI.boys,      # reports coefficients for new covariance matrix
                                                                    vcov = vcovHC(linear.reg.outcome.PSQI.boys,             # estimates a robust covariance matrix
                                                                    type = "HC0",                            # gives White's estimator 
                                                                    cluster = "familyid")),                  # clusters by familyid
                                               conf.int = TRUE, 
                                               conf.level = 0.95)[-c(1),] %>%
...
# table
kable(linear.output.outcome.full.PSQI.boys)
```

```{r PSQI model for girls}
linear.formula.outcome.PSQI.girls <- as.formula(PSQI_global_score_18 ~ class_reordered)

linear.reg.outcome.PSQI.girls <- lm(linear.formula.outcome.PSQI.girls, data = dat.female)

# robust standard error estimation - account for non-independence of twin sample
linear.output.outcome.full.PSQI.girls <- broom::tidy(coeftest(linear.reg.outcome.PSQI.girls,      # reports coefficients for new covariance matrix
                                                                    vcov = vcovHC(linear.reg.outcome.PSQI.girls,             # estimates a robust covariance matrix
                                                                    type = "HC0",                            # gives White's estimator 
                                                                    cluster = "familyid")),                  # clusters by familyid
                                               conf.int = TRUE, 
                                               conf.level = 0.95)[-c(1),] %>%
...
# table
kable(linear.output.outcome.full.PSQI.girls)
```

### Cannabis dependence  

```{r Cannabis model for boys}
linear.formula.outcome.cannabis.boys <- as.formula(cannabis_symptom_scale_18 ~ class_reordered)

linear.reg.outcome.cannabis.boys <- lm(linear.formula.outcome.cannabis.boys, data = dat.male)

# robust standard error estimation - account for non-independence of twin sample
linear.output.outcome.full.cannabis.boys <- broom::tidy(coeftest(linear.reg.outcome.cannabis.boys,      # reports coefficients for new covariance matrix
                                                                    vcov = vcovHC(linear.reg.outcome.cannabis.boys,             # estimates a robust covariance matrix
                                                                    type = "HC0",                            # gives White's estimator 
                                                                    cluster = "familyid")),                  # clusters by familyid
                                               conf.int = TRUE, 
                                               conf.level = 0.95)[-c(1),] %>%
  ...
  mutate(Class = c("Increasing", "Decreasing")) %>% # Create new formatted class variable
  mutate(block = "Physical health block") %>%
  mutate(Variable = "Cannabis dependence") %>%
#  mutate(p.value.adjusted = p.adjust(p.value, method = "fdr", n = 2)) %>%
  mutate(Significance = 
             if_else(
               p.value < 0.05,
...
# table
kable(linear.output.outcome.full.cannabis.boys)
```

```{r Cannabis model for girls}
linear.formula.outcome.cannabis.girls <- as.formula(cannabis_symptom_scale_18 ~ class_reordered)

linear.reg.outcome.cannabis.girls <- lm(linear.formula.outcome.cannabis.girls, data = dat.female)

# robust standard error estimation - account for non-independence of twin sample
linear.output.outcome.full.cannabis.girls <- broom::tidy(coeftest(linear.reg.outcome.cannabis.girls,      # reports coefficients for new covariance matrix
                                                                    vcov = vcovHC(linear.reg.outcome.cannabis.girls,             # estimates a robust covariance matrix
                                                                    type = "HC0",                            # gives White's estimator 
                                                                    cluster = "familyid")),                  # clusters by familyid
                                               conf.int = TRUE, 
                                               conf.level = 0.95)[-c(1),] %>%
...
# table
kable(linear.output.outcome.full.cannabis.girls)
```
