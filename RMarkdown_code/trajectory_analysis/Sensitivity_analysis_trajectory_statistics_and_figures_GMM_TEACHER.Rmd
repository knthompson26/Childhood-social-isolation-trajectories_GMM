...
With just looking at the Mplus output files you can see the following:
- 2 class converges but shows negative variances for S, when S is fixed to zero, I has negative variances. This would then be LCGA 
- 3 class converges but shows negative variances for I, when I is fixed to zero, this ran normally - report these fit statistics and graph. 8 individuals were missing data on all time points. 
- 4 class converges but shows negative variances for I, when I is fixed to zero, this ran normally - report these fit statistics and graph. 8 individuals were missing data on all time points.
- 5 class converges but shows negative variances for I, when I is fixed to zero, this ran normally - report these fit statistics and graph. 8 individuals were missing data on all time points.
- 6 class converges but shows negative variances for I, when I is fixed to zero, this resulted in non-convergence. 
...
si.classes.all <- readModels(paste0(mplus_GMM_clustered_full_TEACHER_output_data_path), recursive = TRUE) #Recursive means it reads data in sub-folders too
```
...
si_summaries <- si_summaries[-c(1,2,4,6,8,9,10),]  # remove rows that are output from OPTSEED tests and keep 5 and 6 trajectory models that fix S to zero
```
...
si.class.summaries <- si.class.summaries %>%
  mutate(
    Model = 
    as.factor(c("Three Class",  #change nrow in data frame above when adding more models
                "Four Class", 
                "Five Class")))  
...
*All models in these statistics is fixing the variance of S (slope) to zero - as the original model produces negative variances.*
...
three_class_si <- as.data.frame(
  si.classes.all$X.Users.katiethompson.Documents.PhD.LISS.DTP_Louise_and_Tim.Social.isolation.trajectories_Paper.1.data_analysis.mplus.GMM.clustered.full_sample.teacher..isolation_3traj_full_sample_clustered_TEACHER_I_fixed_to_zero.out$gh5$means_and_variances_data$y_estimated_means)
...
four_class_si <- as.data.frame(
  si.classes.all$X.Users.katiethompson.Documents.PhD.LISS.DTP_Louise_and_Tim.Social.isolation.trajectories_Paper.1.data_analysis.mplus.GMM.clustered.full_sample.teacher..isolation_4traj_full_sample_clustered_TEACHER_I_fixed_to_zero.out$gh5$means_and_variances_data$y_estimated_means)
...
five_class_si <- as.data.frame(
  si.classes.all$X.Users.katiethompson.Documents.PhD.LISS.DTP_Louise_and_Tim.Social.isolation.trajectories_Paper.1.data_analysis.mplus.GMM.clustered.full_sample.teacher..isolation_5traj_full_sample_clustered_TEACHER_I_fixed_to_zero.out$gh5$means_and_variances_data$y_estimated_means)
...
library(gridExtra)
combined_trajectories_plot_clustered_full_sample <- ggarrange(three_traj_plot_clustered_full_sample, 
                                                              four_traj_plot_clustered_full_sample, 
                                                              five_traj_plot_clustered_full_sample,
                                                              ncol = 3, nrow = 1,
                                                              widths = c(3,3,3), 
...
ggsave(
  "combined_trajectories_plot_clustered_full_sample_TEACHER.jpeg",
