...
quad.mean.two.class
quad.variance.two.class
quad.mean.three.class
quad.variance.three.class
```
...
```{r combine graphs into panel figure}
...
# Check to see how group allocation differs between linear and quadratic 3 class model
...
## Read in class probability file for linear model

```{r read in txt file with probabilities LINEAR, include=FALSE}
...
```{r read in txt file with probabilities QUAD, include=FALSE}
...
table(dat$class_renamed.linear)
...
table(dat$class_renamed.quad)
```

## See how many people were classed the same way
...
table(dat$matching.class)
```
78 people who were not classified the same way.
...
table(dat$matching.isolated)
table(dat$class_renamed.linear)
table(dat$class_renamed.quad)
```
Overall average movement: 
- 1  more person in the increasing class
- 18 less people in the decreasing class
- 17 more people in the low stable class

88 of 117 in the decreasing class are classified the same.
87 of 106 in the increasing class are classified the same.
1979 of 2009 in the low stable class are classified the same. 
...
table(dat.non.match$direction.match)
```
