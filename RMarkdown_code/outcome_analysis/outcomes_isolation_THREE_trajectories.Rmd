---
title: "Associations between social isolation trajectories and outcomes at age 18"
output:  
  html_document:
    df_print: paged
    toc: yes
...
```{r lists for outcome regressions, include=FALSE}
...
* add.significance.variable() add's a variable to the dataset that labels if a result has a significant p value or not. 
* paper.table() creates a table that is formatted correctly for either linear or logitic regression results. 
* create.dataset() binds regression results together to form a data set. 
* huber.white.cluster.logistic() applies heteroskedasticity-robust standard errors while clustering for familyid for logistic regression
* order.variables() orders the data whereby points in a plot are arranged by OR/Beta and domain. 
* reorder_cor_matrix() orders the correlation matrix for the heat map
* huber.white.cluster.linear() applies heteroskedasticity-robust standard errors while clustering for familyid for linear regression
...
```{r posterior probability indicator, class.source = 'fold-show'}
posterior.cut = FALSE
```
...
```{r logistic regression loop, results='hide', class.source = 'fold-show'}
...
```{r multiple testing correction for logistic models, include=FALSE}
### Getting multiple testing correction (BH FDR) p values -- IGNORE -- **FDR correction not applied here.** 
data.outcome.logistic.decreasing.fdr <- multiple.testing(data = data.outcome.logistic,
                                                          class = "Decreasing",
                                                          method = "BH",
                                                          n.tests = 11)

data.outcome.logistic.increasing.fdr <- multiple.testing(data = data.outcome.logistic,
                                                          class = "Increasing",
                                                          method = "BH",
                                                          n.tests = 11)
  
data.outcome.logistic.fdr <- rbind(
  data.outcome.logistic.decreasing.fdr,
  data.outcome.logistic.increasing.fdr
)
# variables that were significant that become non-significant
data.outcome.logistic.fdr %>%
...
```{r plot made for the SGDP presentation, include=FALSE}
...
```{r save logistic plot, include=FALSE}
...
```{r linear regression loop with z scores, results='hide', class.source = 'fold-show'}
...
```{r multiple testing correction for linear models, include=FALSE}
### Getting multiple testing correction (BH FDR) p values -- IGNORE -- **FDR correction not applied here.** 
...
# variables that were significant that become non-significant
data.outcome.linear.zscore.fdr %>%
  filter(Significance == "Significant" & Significance.adjusted == "Non-significant")
```

```{r table for multiple corrected, include=FALSE}
...
```{r save linear individual plot zscore, include=FALSE}
# if(posterior.cut == FALSE){
# # save
# ggsave(
...
```{r linear regression outcome plot zscores - SGDP presentation, include=FALSE}
...
# Controlling for significant antecedents {.tabset .tabset-fade}
...
## Logistic regression: controlling for antecedents {.tabset .tabset-fade}

### Loop

```{r logistic regression loop controlling for antecedents, class.source = 'fold-show'}
...
```{r multiple testing correction for logistic models ant control, include=FALSE}
### Getting multiple testing correction (BH FDR) p values -- IGNORE -- **FDR correction not applied here.** 
data.outcome.logistic.decreasing.ant_control.fdr <- multiple.testing(data = data.outcome.logistic.ant_control,
                                                          class = "Decreasing",
                                                          method = "BH",
                                                          n.tests = 11)

data.outcome.logistic.increasing.ant_control.fdr <- multiple.testing(data = data.outcome.logistic.ant_control,
                                                          class = "Increasing",
                                                          method = "BH",
                                                          n.tests = 11)
  
data.outcome.logistic.ant_control.fdr <- rbind(
  data.outcome.logistic.decreasing.ant_control.fdr,
  data.outcome.logistic.increasing.ant_control.fdr
)
# variables that were significant that become non-significant
data.outcome.logistic.ant_control.fdr %>%
  filter(Significance == "Significant" & Significance.adjusted == "Non-significant")
```

### Table 

```{r table for combined logistic dataframes controlling for antecedents}
...
## Linear regression: controlling for antecedents {.tabset .tabset-fade}

### Loop

```{r linear regression loop zscore ant control, class.source = 'fold-show'}
...
```{r multiple testing correction for linear models ant control, include=FALSE}
### Getting multiple testing correction (BH FDR) p values -- IGNORE -- **FDR correction not applied here.** 
data.outcome.linear.zscore.decreasing.ant_control.fdr <- multiple.testing(data = data.outcome.linear.zscore.ant_control,
                                                          class = "Decreasing",
                                                          method = "BH",
                                                          n.tests = 20)

data.outcome.linear.zscore.increasing.ant_control.fdr <- multiple.testing(data = data.outcome.linear.zscore.ant_control,
                                                          class = "Increasing",
                                                          method = "BH",
                                                          n.tests = 20)
  
data.outcome.linear.zscore.ant_control.fdr <- rbind(
  data.outcome.linear.zscore.decreasing.ant_control.fdr,
  data.outcome.linear.zscore.increasing.ant_control.fdr
)
# Originally significant variables 
data.outcome.linear.zscore.ant_control.fdr %>%
  filter(Significance == "Significant")
# variables that were significant that become non-significant
data.outcome.linear.zscore.ant_control.fdr %>%
  filter(Significance == "Significant" & Significance.adjusted == "Non-significant")
# Significant in both
data.outcome.linear.zscore.ant_control.fdr %>%
  filter(Significance == "Significant" & Significance.adjusted == "Significant")
```

### Table

```{r table for combined linear dataframes zscore ant control}
...
```{r linear regression outcome plot zscore ant control for sgdp center talk, include=FALSE}
...
```{r save SGDP separate plot, include=FALSE}
if(posterior.cut == FALSE){
ggsave(
