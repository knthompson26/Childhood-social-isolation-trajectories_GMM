...
Forty variables was con variables is too many to include. Variables that have been dropped for the 3 class analysis:
...
All regressions are first computed in STATA - multinomial regression with clustering is not currently doable in R. The coefficients are then imported into R to be interpreted. 
...
2. Significant only regressions. This is the full model of antecedents that only includes variables that were significant at the individual regression stage. (**THIS IS NOT USED IN THE PAPER**)
...
## Multivariate model including variables that were significant in univariate models (**THIS IS NOT USED IN THE PAPER**)
