...
Antecedents that remained significant in the full antecedent model:child prosocial behaviours, ADHD behaviours, internalising behaviours, and child executive function 
...
for (i in 1:length(outcome.dependent.logistic))
{ cat(i)
  logistic.formula.outcome.ant_control <- as.formula(sprintf("%s ~ class_reordered + prosocial_behaviours_combined_05 + ADHD_combined_05 + internalising_combined_excl_sis_05 + executive_function_05", outcome.dependent.logistic[i]))

  logistic.reg.outcome.ant_control <- glm(logistic.formula.outcome.ant_control, data = dat, family = "binomial")

  logistic.output.outcome.full.ant_control <- broom::tidy(logistic.reg.outcome.ant_control, conf.int = TRUE, exponentiate = TRUE, conf.level = 0.95)[-c(1,4:7),] %>%
    dplyr::rename(
           OR = estimate,
           OR.conf.low = conf.low,
           OR.conf.high = conf.high)
...
for (i in 1:length(outcome.dependent.logistic))
{ 
  assign(paste0("logistic.", names(logistic.stats.outcome.ant_control[i]), ".outcome"), as.tibble(logistic.stats.outcome.ant_control[[i]]))
}
```
...
for (i in 1:length(outcome.dependent.linear.zscore))
{ cat(i)
  linear.formula.outcome.zscore.ant_control <- as.formula(sprintf("%s ~ class_reordered + prosocial_behaviours_combined_05 + ADHD_combined_05 + internalising_combined_excl_sis_05 + executive_function_05", outcome.dependent.linear.zscore[i]))

  linear.reg.outcome.zscore.ant_control <- lm(linear.formula.outcome.zscore.ant_control, data = dat)

  linear.output.outcome.full.zscore.ant_control <- broom::tidy(linear.reg.outcome.zscore.ant_control, conf.int = TRUE, conf.level = 0.95)[-c(1,4:7),] 
...
for (i in 1:length(outcome.dependent.linear.zscore))
{ 
  assign(paste0("linear.", names(linear.stats.outcome.zscore.ant_control[i]), ".outcome"), as.tibble(linear.stats.outcome.zscore.ant_control[[i]]))
}
```
...
Plot the linear regression results - controlling for antecedents
...
## Logistic regression loop  - sex differences
...
```
Significant interaction for "Not in employment or education" for both increasing and decreasing classes. 

## Linear regression loop - sex differences
...
```
Significant interactions for BMI and the increasing class. 

# Zygosity differences

## Logistic regression loop  - zygosity differences
...
```
## Linear regression loop - zygosity differences
...
No significant zygosity differences. 
...
for (i in 1:length(outcome.dependent.logistic))
{ cat(i)
  logistic.formula.outcome.pairwise.ant_control <- as.formula(sprintf("%s ~ class_binary + prosocial_behaviours_combined_05 + ADHD_combined_05 + internalising_combined_excl_sis_05 + executive_function_05", outcome.dependent.logistic[i]))
...
for (i in 1:length(outcome.dependent.logistic))
{ 
  assign(paste0("logistic.pairwise.", names(logistic.stats.outcome.pairwise.ant_control[i]), ".outcome"), as.tibble(logistic.stats.outcome.pairwise.ant_control[[i]]))
}
```
...
outcome.logistic.separate.pairwise.plot.ant_control <- ggplot(
  data.outcome.logistic.pairwise.ant_control, # CHANGE DEPENDING IF YOU WANT TO USE Z SCORES OR NOT - remember to rename the plot if you change it
  aes(x = `Dependent variable`,
      y = OR,
      ymin = OR.conf.low,
      ymax = OR.conf.high,
...
for (i in 1:length(outcome.dependent.linear.zscore))
{ cat(i)
  linear.formula.outcome.zscore.pairwise.ant_control <- as.formula(sprintf("%s ~ class_binary + prosocial_behaviours_combined_05 + ADHD_combined_05 + internalising_combined_excl_sis_05 + executive_function_05", outcome.dependent.linear.zscore[i]))
...
for (i in 1:length(outcome.dependent.linear.zscore))
{ 
  assign(paste0("linear.pairwise.", names(linear.stats.outcome.zscore.pairwise.ant_control[i]), ".outcome.ant_control"), as.tibble(linear.stats.outcome.zscore.pairwise.ant_control[[i]]))
}
```
