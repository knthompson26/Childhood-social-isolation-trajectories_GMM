...
for (i in 1:length(outcome.dependent.linear.zscore))
{ 
  assign(paste0("linear.zscore.", names(linear.stats.outcome.zscore[i]), ".outcome"), as.tibble(linear.stats.outcome.zscore[[i]]))
}
```
...
outcome.linear.separate.plot.zscore <- ggplot(
 ...
 scale_y_continuous(breaks=c(-1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2)) + # log10_trans() has been removed here
  coord_flip()
