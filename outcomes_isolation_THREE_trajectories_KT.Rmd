...
outcome.linear.separate.plot <- ggplot(
  data.outcome.linear, # CHANGE DEPENDING IF YOU WANT TO USE Z SCORES OR NOT - remember to rename the plot if you change it
  aes(x = `Dependent variable`,
      y = estimate,
      ymin = conf.low,
      ymax = conf.high,
 ...
 # scale_y_continuous(trans = scales::log10_trans(), breaks=c(0.2,0.4,0.6,0.8, 1, 1.5, 2, 2.5, 3, 4, 5)) + # should this be plotted this way - or should this be a normal scale?
  coord_flip()

outcome.linear.separate.plot

#save
ggsave(
  "linear_outcome_individual_regressions.png",
  plot = outcome.linear.separate.plot,
  device = "png",
  path = paste0(graph_save_data_path, "outcomes/analysis_interpretation"),
  width = 12,
  height = 9
