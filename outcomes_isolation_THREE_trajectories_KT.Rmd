...
outcome.dependent.linear.name.list <- c("Current depression",
                                        "Current anxiety",
                                        "Current inattentive and hyperactive symptoms",
                                        "Current conduct disorder symptoms",
                                        "Current alcohol dependence symptoms",
...
{ cat(i)
  ...
  multinom.output.ant.full <- broom::tidy(linear.reg.outcome, conf.int = TRUE, conf.level = 0.95)[-c(1,3),] 
  ...
  linear.stats.outcome[[i]] <- multinom.output.ant.full
}
# set the names of the list to match the variable names
names(linear.stats.outcome) <- antecedent.independent
```
